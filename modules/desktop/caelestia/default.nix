...
# rice.shell = "caelestia" iken aktif (bkz. shells/choice.nix);
# kabuktan bağımsız parçalar shells/common.nix'te.
{ inputs, lib, pkgs, config, ... }:

let
  system = pkgs.stdenv.hostPlatform.system;

  # Şema geçişi sonrası koşan hook: pywalfox'u tetikler (Firefox canlı boyanır)
...
in
{
  ...
  config = lib.mkIf (config.rice.shell == "caelestia") {
    programs.caelestia = {
      enable = true;
    ...
    # --- Guard: btop tema seçimi (yalnızca config yoksa; btop config'ini
    ...
    # --- Mutable-copy hilesi: runtime'da yazılan HM-yönetimli config ---
    ...
    # (vesktop settings.json eşdeğerleri modules/apps/vesktop.nix'te)
