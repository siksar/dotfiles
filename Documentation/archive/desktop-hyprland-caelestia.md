# ARŞİV — Hyprland masaüstü / Caelestia kabuğu (09 Ağu – Eyl 2026)

> **DONMUŞ (11 Eyl 2026).** Bu defter, ağaçtan çıkmış bir masaüstünü anlatır:
> Hyprland + Caelestia + Serpantinum ve o dönemin giriş ekranı (ly → SDDM).
> İçindeki yol adları (`home/desktop/…`, `system/desktop/session.nix`) ve
> durumlar **kasıtlı olarak eskidir** — "düzeltme". Neden korunuyor: tasarım
> gerekçeleri (dGPU DRM guard mantığı, güç bütçesi kuralı, süsleme politikası
> araştırması) hâlâ öğretici.
>
> **Güncel masaüstü belgesi: `Documentation/desktop.md` (COSMIC + Plasma).**
...
tamamen `system/kernel/power.nix`'teki logind zincirine bırakıldı. Bu kasıtlı:
Caelestia'nın oturum menüsü zaten `systemctl`/`loginctl` çağrılarını logind D-Bus'a
alias'lıyor ve `hibernate` komutu `SessionManager.suspendThenHibernate`'e eşleniyor
(`CanHibernate` ön kontrolüyle, kullanılamazsa düz suspend'e düşüyor).

**GÜNCELLEME 24 Ağu 2026 — zincir artık düz `suspend`.** `suspend-then-hibernate`
(25 dk `HibernateDelaySec`) kaldırıldı: hibernate'in dondurma aşaması amdgpu/TTM
defterini bozuyor ve sistem saatler sonra oyun sırasında kurtarılamaz kilitleniyordu
(kanıt: `Documentation/aerox16/power.md`, "Hibernate neden kapatıldı"). Menü tarafında
ek koruma yine gerekmedi — ama sebebi değişti: `nohibernate` kernel parametresi
`CanHibernate`'i "na" yaptığı için menünün `hibernate` komutu kendi ön kontrolüyle
düz suspend'e düşüyor. Yani koruma artık "menüde düz suspend çağrısı yok"tan değil,
"hibernate yeteneği hiç yok"tan geliyor.
...
| SUPER+SHIFT+E | oturumu kapat (SDDM giriş ekranına döner) |
...
3. `reboot` → SDDM → **Hyprland (uwsm-managed)** seç (`defaultSession` zaten
   bunu işaret ediyor). SDDM/UWSM/Hyprland/Caelestia hepsi temiz başlar.
...
- **Idle** — Caelestia'nın kendi `ext-idle-notify-v1` yöneticisi: 5 dk boşta
  ...
  kararı logind'e bırakıldı (`system/kernel/power.nix`). 24 Ağu 2026'dan beri
  zincir düz `suspend` (s2idle); hibernate `nohibernate` ile kapalı, menünün
  `hibernate` komutu `CanHibernate` "na" görüp düz suspend'e düşüyor.
...
## Giriş ekranı — ly'den SDDM'e (25 Ağu 2026)

Greeter artık **SDDM**, Wayland kipinde, compositor'u **kwin**
(`system/desktop/login.nix`). ly'nin TTY greeter'ının yerini aldı.

**Neden.** Plasma 6 sisteme girince metin modunda bir kutu ile tam Wayland kabuğu
arasındaki kopukluk kalıcı hâle geldi. Teknik olarak da yeni bir yığın eklenmiyor:
plasma6 modülü `sddm`'in `package` / `theme` / `wayland` alanlarını **zaten**
dolduruyordu, yalnız `enable`'a dokunmuyordu — geçiş tek anahtar çevirmek.

**Neyi kaybettik, bilerek.** ly TTY üstünde koşuyordu (fbcon → amdgpu) ve dGPU'nun
DRM node'una hiç dokunmuyordu. SDDM'in Wayland greeter'ı bir `kwin_wayland` örneği
başlatır; kwin varsayılan olarak **tüm** DRM kartlarını açar, yani NVIDIA dGPU giriş
ekranı boyunca D3cold'a giremezdi. Bedeli `KWIN_DRM_DEVICES=/dev/dri/sddm-igpu` ile
sıfırlıyoruz. **Bu maliyet zaten yalnız login ekranındaydı** — oturum açılınca greeter
ölür, dolayısıyla 4.28W *oturum içi* idle bütçesi bu değişiklikten etkilenmiyor.

### Üç yük taşıyan ayrıntı

| Ne | Neden böyle |
|---|---|
| `settings.General.GreeterEnvironment` | **Ekleme değil, DEĞİŞTİRME.** Modülün `defaultConfig`'i kwin seçiliyken oraya `QT_WAYLAND_SHELL_INTEGRATION=layer-shell` yazar; `cfgFile = recursiveUpdate defaultConfig settings` olduğu için buradaki tanım satırın tamamını ezer. layer-shell elle tekrar edilmezse greeter kwin altında konumlanamaz. |
| `KWIN_DRM_DEVICES` → **kendi** udev symlink'i (`sddm-igpu`) | `plasma.nix`'in `kwin-igpu`'su `desktop.plasma.enable`'ın `mkIf`'i **içinde**. Ödünç alınsaydı Plasma kapatıldığı an greeter var olmayan bir node'a bakar, kwin hiç GPU bulamaz ve **giriş ekranı hiç açılmaz**. Greeter hiçbir oturumun bayrağına bağlı olmamalı. Değer `:` içeremez — `session.nix`'teki `AQ_DRM_DEVICES` ile birebir aynı tuzak. |
| `theme` = **mutlak store yolu** | Stylix'in `sddm` hedefi **yok**. Breeze `kdePackages.plasma-desktop`'tan (plasma-workspace'ten değil) kopyalanıp `theme.conf`'un yalnız iki anahtarı sed'leniyor: `background` → `config.stylix.image`, `color` → base0D. Mutlak yol çalışıyor çünkü `Display.cpp` onu `QDir::absoluteFilePath`'ten geçiriyor; kazancı temanın `systemPackages`'a girmemesi (Caelestia profiline sızmıyor). Yan etki: yol literal `"breeze"` olmadığı için modülün koşullu imleç bloğu çalışmaz — `Theme.CursorTheme`/`CursorSize` elle, yine Stylix'ten veriliyor. |

Değişkenin gerçekten kwin'e ulaştığı sddm kaynağında izlendi: `GreeterEnvironment`
girdileri `Backend.cpp`'de `XDG_SESSION_CLASS=greeter` iken sürecin ortamına ekleniyor,
o ortam da `UserSession.cpp`'de `sddm-helper-start-wayland <compositor> <greeter>` ile
başlatılan sürece miras kalıyor.

### Etkilenmeyenler (doğrulandı)

- **Anahtarlık otomatik kilit açma.** Zincir ly'ye değil, gnome-keyring modülüne bağlı:
  o `security.pam.services.login.enableGnomeKeyring` yazıyor, `/etc/pam.d/sddm` de dört
  yönetim grubunda `login`'i include/substack ediyor. `kwallet-pam` de aynı zincirde
  (plasma6'dan).
- **`display-manager.service`.** ly de SDDM de `services.displayManager.generic`
  üzerinden aynı unit'i kuruyor, yalnız `execCmd` değişiyor. `power-display.nix`'in
  `graphical.target`'a asılma gerekçesi ve döngü notu aynen geçerli.
- **`defaultSession = "hyprland-uwsm"`.** SDDM bunu `DefaultSession=hyprland-uwsm.desktop`
  olarak yazıyor; Caelestia varsayılan kalıyor.

### Geri dönüş

ly yapılandırması (1.4.1, `0xSSRRGGBB` truecolor + `bigclock`) git geçmişinde:
`login.nix`'in 25 Ağu 2026 öncesi hâli. `services.displayManager.ly.enable` ile
`sddm.enable` birbirini dışlar — ikisi de `display-manager.service`'i sahiplenir.
...
greeter'da **karantinalı üçüncü bir oturum** olarak kurar. Caelestia
