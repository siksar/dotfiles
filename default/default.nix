# default.nix - Backward compatibility for non-flake commands
# For flake users: Use `nixos-rebuild switch --flake .#nixos`

# This file enables `nix-build`, `nix-env`, and `nix-shell` commands
# to work without explicit flake references.

let
	# Try to read flake.lock if it exists, otherwise fail gracefully or use strict version
	lock = if builtins.pathExists ./flake.lock 
		then builtins.fromJSON (builtins.readFile ./flake.lock)
		else { nodes.nixpkgs.locked.rev = "nixos-unstable"; nodes.nixpkgs.locked.narHash = "sha256-00000000000000000000000000000000000000000000"; };
		
	nixpkgs = fetchTarball {
		url = "https://github.com/NixOS/nixpkgs/archive/${lock.nodes.nixpkgs.locked.rev}.tar.gz";
		sha256 = lock.nodes.nixpkgs.locked.narHash;
	};
	pkgs = import nixpkgs {
		config.allowUnfree = true;
	};
in
{
	# Re-export pkgs for nix-shell compatibility
	inherit pkgs;
  
	# Import configuration.nix for direct access
	# Adjusted path to point to base/configuration.nix
	configuration = import ../base/configuration.nix {
		inherit pkgs;
		config = {};
		lib = pkgs.lib;
		inputs = {};  # Note: inputs won't be available in non-flake mode
	};
}
