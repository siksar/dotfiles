...
- **Waybar's 16-theme system** (`waybar-themes.nix`, `waybar-mono*.css`,
  `waybar-omarchy-compat.nix`, `waybar-themes/<V>/`) runs themes straight from the
...
- Any `@import` rewrite inside `waybar-themes.nix`'s `mkTheme` must stay a relative
...
- **rofi 2.0 cannot parse `@variable` inside `linear-gradient(...)`** — and it does not
  ...
  built-in Solarized **light** theme. This is why `rofi-themes.nix` bakes literal hexes
