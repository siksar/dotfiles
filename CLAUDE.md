...
### Desktop: GNOME + ly (display manager), themed by Stylix

The desktop is stock GNOME (`modules/desktop/gnome.nix`: `services.desktopManager.gnome`,
a lean `environment.gnome.excludePackages` list) with user-side dconf settings in
`modules/desktop/gnome-hm.nix` (a minimal keybinding set, plus the mutter
`variable-refresh-rate` experimental feature and the wallpaper symlink). An older
Hyprland + Caelestia/dms/noctalia + SDDM rice lives on the `rice/caelestia` branch;
the current Hyprland rice is an in-tree opt-in module (next section).

The **display manager is ly** (`modules/desktop/ly.nix`), a TUI greeter on the TTY —
replaced GDM (2026-07-18) for a lighter greeter and a clean picker across the GNOME +
opt-in Hyprland sessions. ly 1.4.1 takes 32-bit `0xSSRRGGBB` truecolor, so it's themed
by feeding `config.lib.stylix.colors` (kanagawa-dragon) directly into
`services.displayManager.ly.settings` (Stylix has no ly target). GNOME officially blesses
only GDM for its Wayland session; launching GNOME-Wayland from ly works but is slightly
unofficial — revert by dropping `ly.nix` and re-adding `services.displayManager.gdm.enable`
in `gnome.nix`.
...
Stylix targets auto-theme GTK/GNOME, ghostty, vscodium, vesktop, starship, etc. — don't set
...
SUPER+T wallpaper→matugen theme chain, why `withUWSM = true` is mandatory (the
session file exists even without it but fails with "Unit not found"), the Lua
config integration — is `docs/hyprland-rice.md`; read it before touching the rice.
...
### Opt-in Sway rice (`modules/desktop/sway-rice/`)

A third, fully independent desktop session — Sway + noctalia-shell (bar, notifications,
control center, launcher, clipboard, OSD, lock screen, idle, wallpaper all in one) — a
declarative port of vyrx-dev/dotfiles, toggled by `rice.sway.enable = true;` in
`configuration.nix` (currently on). Does **not** replace GNOME or the Hyprland rice; ly
gains a "Sway" entry. Deliberately shares **no code** with the Hyprland rice — the user
asked for independence over DRY. Full design doc, deviations from upstream vyrx, and the
complete keybind table (Mod+G opens a searchable cheatsheet built from the same source
list) live in `docs/sway-rice.md`; read it before touching the rice. The rice-internal
gotchas (`keybinds.nix` as single source, `WLR_DRM_DEVICES` placement, the
`stylix.targets.noctalia` conflict, the noctalia session-target override) live in
`modules/desktop/sway-rice/CLAUDE.md`, which loads automatically when you work in that
directory.

One rule stays here because it reaches outside the rice: `rice.sway.enable` also flips
`users.users.zixar.shell = pkgs.fish` (in `sway-rice/system.nix`), changing the login
shell for *every* session including GNOME. It is the riskiest single line in the rice —
after touching it, verify `systemctl --user status home-manager-zixar.service` is green
and a GNOME login still works (see commit 0f0be6c for the failure mode).

**Both rices share one contract:** the HM side follows the system flag automatically via
`osConfig` — never add a second toggle. This works on both the embedded and standalone HM
paths only because of the flake's `extraSpecialArgs.osConfig` pass-through, so changes to
`flake.nix` can silently break either rice.
...
platform-profile layer is **power-profiles-daemon** (`power.nix`, enables the GNOME power
slider) — it **replaced TLP** (2026-07-18): TLP's simultaneous governor+EPP+platform_profile
writes fought amd-pstate=active (AMD's own guidance), and its device tweaks (USB autosuspend,
WiFi PS, runtime PM) are now left to powertop --auto-tune + the kernel ASPM param + the EC.
...
TLP's job) and there's no GNOME power slider under Hyprland, so power-saver's 2.0 GHz cap would
